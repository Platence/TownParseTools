package interfaces;

public class PathsAndCellAdress {

    public final static String PATH_READ_FILE = "?????";
    public static final String PATH_SAVES = "?????";
    // Type your path to FILE

    public static final int SHEET_TOWN_INDEX = 3;
    public static final int CELL_AREA_CELL = 6;
    public static final int CELL_TOWN_CELL = 7;
    public static final int CELL_INDEX = 8;
    public static final int CELL_STREET = 9;
    public static final int SHEET_PERSON_INDEX = 0;
    public static final int CELL_FIRTST_NAME = 0;
    public static final int CELL_DEFOLT_CITY = 2;

    public static final int MAX_LENGTH_MES = 20;
    public static final int MAX_TRY_REQ    = 15;
    public static final String DEFAULT_KM = "-99999";

    public static final String DEF_KEY_ERROR   = "Включите JavaScript";
    public static final String DEF_KEY_KEY     = "км";                  // Danger use this word!
    public static final String DEF_KEY_READY   = "ready";
    public static final String DEF_KM_TOWN_OWN = "10";




}
